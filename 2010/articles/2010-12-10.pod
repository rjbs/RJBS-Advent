...
since moved on to other ventures.  Everybody shop this kind of system: it
...
Most in the M4 file were macros.  That meant that when you said, "XYZ sends
...
easy, since they're all pretty much the same.  We stored our host definitions
in YAML files, like this one:
...
puts all the Solaris zones on a given server in the same file with the server.
The tool doesn't care.  It just reads in a bunch of YAML and turns it into a
Host object.  When every host is loaded in, the hosts can be turned into DNS
data lines with their C<as_data_lines> method:
